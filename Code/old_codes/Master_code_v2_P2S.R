...
Dataset = "old"
working_dir <- "D:/Nestle/Emerging/"
current_date <- format(Sys.Date(), "%d%m%Y")
results_dir <- str_glue("Results_{current_date}")
...
OneSpace_Claims_Aggr <- OneSpace_Claims.Nielsen %>% select(c(UPC, CLAIM_TEXT))
...
if (Dataset == "old"){
  ...
  CLAIMDIM <- readRDS(file = paste0(getwd(), "/Emerging/","CLAIMDIM_03102020.rda"))
  CLAIMFACTs <- readRDS(file = paste0(getwd(), "/Emerging/","CLAIMFACTs_03102020.rda"))
  PRODUCTDIM <- readRDS(file = paste0(getwd(), "/Emerging/","PRODUCTDIM_03102020.rda"))
  ...
  colnames(Mapped_data_from_Nestle) <- c("UPC", "Claim.Name")
  
  Mapped_data_from_Nestle$Nestle.Mapping.flag <- 1
  
  
  Mapped_Nestle_dcasted <- reshape2::dcast(Mapped_data_from_Nestle, UPC ~ Claim.Name, value.var = "Nestle.Mapping.flag")
  colnames(Mapped_Nestle_dcasted)[2:ncol(Mapped_Nestle_dcasted)] <- paste0("Nestle_", colnames(Mapped_Nestle_dcasted)[2:ncol(Mapped_Nestle_dcasted)])
  Mapped_Nestle_dcasted <- Mapped_Nestle_dcasted[unique(Mapped_Nestle_dcasted$UPC) %in% 
                                                   unique(OneSpace_Claims_Aggr$UPC),]
...
source("Emerging_functions_3_optimized_prefix.suffix.R")
...
write.csv(OneSpace_Claims_Aggr,"preprocessed_onespace.csv",row.names = F)
write.csv(Nielsen_Dt,"preprocessed_Nestle.csv",row.names = F)
write.csv(TaxoNomy,"preprocessed_Taxonomy.csv",row.names = F)
